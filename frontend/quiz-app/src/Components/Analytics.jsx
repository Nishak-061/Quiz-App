...
const Analytics = () => {
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`https://quiz-app-backend-s3ov.onrender.com/api/quizzes/${quizToDelete}`);
  ...
  const handleEditQuiz = async (quiz) => {
    try {
      if (quiz.type === "Q&A") {
        ...
        const response = await axios.get(
          `https://quiz-app-backend-s3ov.onrender.com/api/qanda/get-quiz/${uniqueLink}`
        );
        ...
        const uniqueLink = linkParts[linkParts.length - 1];
        const response = await axios.get(
          `https://quiz-app-backend-s3ov.onrender.com/api/pollQuiz/get-quiz/${uniqueLink}`
        );
